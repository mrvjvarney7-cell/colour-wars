...
def main():
    ...
    n_promoted = sum(1 for r in history if r["promoted"])
    total_hours = sum(r["iter_time_sec"] for r in history) / 3600
    ...
    if warning:
        ...
        if len(history) >= STAGNATION_WINDOW:
            recent = [r["win_rate_vs_best"] for r in history[-STAGNATION_WINDOW:]]
